...
const AdminLoginCard = () => {
    ...
    const handleSubmit = async(e) => {
        ...
        if (mobile === '') {
            setErrorMsg("Enter admin mobile number")
            setSuccessMsg('')
            return
        }
    ...
    return(
        ...
        <Box sx={{bgcolor: '#fff', m: 1, px: 3, py: 3, width: {xs: '300px' , md:'410px', xl: '550px'}, minHeight: {xs:'320px', md:'420px', xl: '420px'}, display: 'flex', flexDirection:'column', justifyContent: 'center', borderRadius: '15px', boxShadow: "0 8px 24px rgba(0,0,0,0.08)",}} variant="outlined">
            ...
            <Typography sx={{mb: "6px", mt: '6px', fontSize: "14px", fontWeight: 500, color: "#475569",}} >Admin Mobile Number</Typography>
            <TextField
                placeholder="Enter registered admin mobile number"
                type='number'
                fullWidth
                value={mobile}
                onChange={(e) => {
            ...
            <Typography sx={{mt: 2, fontSize: "14px", textAlign: "center", color: "#475569",}}>
                <span onClick={() => navigate("/login")} style={{ color: "#0b69ff", cursor: "pointer" }}>Back to User Login</span>
            </Typography>
        </Box>
        </form>
    )
